...
const { removeListener } = require('../models/UserSchema');
...
router.post('/register', async (req, res) => {
  ...
  if (!rollNo || !password || !role)
    return res
      .status(400)
      .json({ msg: 'Password , email and role are required' });

  const user = await Users.findOne({ rollNo, role }); // finding user in db
...
router.get('/logout', async (req, res) => {
  if (req.session.user) {
    req.session.destroy((err) => {
      ...
      else return res.status(200).json({ msg: 'Logout Successfull...' });
    });
  }
});
